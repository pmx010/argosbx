...
if [ "$1" = "rep" ]; then
[ "$vwp" = yes ] || [ "$sop" = yes ] || [ "$vxp" = yes ] || [ "$ssp" = yes ] || [ "$vlp" = yes ] || [ "$vmp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] || [ "$xhp" = yes ] || [ "$anp" = yes ] || [ "$arp" = yes ] || { echo "Hint: When resetting protocols with rep, please set at least one protocol variable before the script. Bye! 💣"; exit; }
fi
else
[ "$1" = "del" ] || [ "$vwp" = yes ] || [ "$sop" = yes ] || [ "$vxp" = yes ] || [ "$ssp" = yes ] || [ "$vlp" = yes ] || [ "$vmp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] || [ "$xhp" = yes ] || [ "$anp" = yes ] || [ "$arp" = yes ] || { echo "Hint: Argosbx script not installed. Please set at least one protocol variable before the script. Bye! 💣"; exit; }
fi
export uuid=${uuid:-''}
...
showmode(){
echo "Argosbx Script One-Click SSH Command Generator Online URL: https://yonggekkk.github.io/argosbx/"
echo "Main Script: bash <(curl -Ls https://raw.githubusercontent.com/yonggekkk/argosbx/main/argosbx.sh) OR bash <(wget -qO- https://raw.githubusercontent.com/yonggekkk/argosbx/main/argosbx.sh)"
echo "Command to show node info: agsbx list [OR] Main Script list"
echo "Command to reset variable groups: Define protocol variables agsbx rep [OR] Define protocol variables Main Script rep"
echo "Command to update script: Previously installed protocol variables Main Script rep"
echo "Command to update Xray or Singbox core: agsbx upx or ups [OR] Main Script upx or ups"
echo "Command to restart script: agsbx res [OR] Main Script res"
echo "Command to uninstall script: agsbx del [OR] Main Script del"
echo "Dual-stack VPS show IPv4/IPv6 node config: ippz=4 or 6 agsbx list [OR] ippz=4 or 6 Main Script list"
echo "---------------------------------------------------------"
echo
}
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
echo "Argosbx One-Click Non-Interactive Lightweight Script 💣"
echo "Current Version: V25.11.20"
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
echo
echo "All node name prefix: $name"
fi
v4v6
...
echo; echo "Please note: You have installed warp"
else
if [ "$wap" != yes ]; then
s1outtag=direct; s2outtag=direct; x1outtag=direct; x2outtag=direct; xip='"::/0", "0.0.0.0/0"'; sip='"::/0", "0.0.0.0/0"'; wap=warpargo
else
...
echo "Installed Xray official core: $sbcore"
}
upsingbox(){
...
echo "Installed Sing-box official core: $sbcore"
}
insuuid(){
...
echo "Vless-xhttp-reality-enc Port: $port_xh"
cat >> "$HOME/agsbx/xr.json" <<EOF
    {
      "tag":"xhttp-reality",
      "listen": "::",
...
echo "Vless-xhttp-enc Port: $port_vx"
if [ -n "$cdnym" ]; then
echo "$cdnym" > "$HOME/agsbx/cdnym"
echo "Host domain for 80 series CDN or origin CDN (Ensure IP is resolved to CF domain): $cdnym"
fi
cat >> "$HOME/agsbx/xr.json" <<EOF
    {
      "tag":"vless-xhttp",
...
echo "Vless-ws-enc Port: $port_vw"
if [ -n "$cdnym" ]; then
echo "$cdnym" > "$HOME/agsbx/cdnym"
echo "Host domain for 80 series CDN or origin CDN (Ensure IP is resolved to CF domain): $cdnym"
fi
...
echo "Vless-tcp-reality-v Port: $port_vl_re"
cat >> "$HOME/agsbx/xr.json" <<EOF
        {
            "tag":"reality-vision",
            "listen": "::",
...
echo "Shadowsocks-2022 Port: $port_ss"
cat >> "$HOME/agsbx/sb.json" <<EOF
        {
            "type": "shadowsocks",
            "tag":"ss-2022",
...
rc-update addxray default >/dev/null 2>&1
...
echo "Downloading Cloudflared-argo latest official core: $argocore"
...
echo "Applying for Argo $argoname tunnel... Please wait"
sleep 8
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
...
echo "Argo $argoname tunnel application successful"
else
echo "Argo $argoname tunnel application failed, please try again later"
fi
fi
sleep 5
echo
...
echo "if ! find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -Eq 'agsbx/(s|x)' && ! pgrep -f 'agsbx/(s|x)' >/dev/null 2>&1; then echo 'System interruption or variable format error detected? It is recommended to enter reboot in the SSH dialog to restart the server. Automatically performing Argosbx script node recovery operation, please wait...'; sleep 6; export cdnym=\"${cdnym}\" name=\"${name}\" ippz=\"${ippz}\" argo=\"${argo}\" uuid=\"${uuid}\" $wap=\"${warp}\" $xhp=\"${port_xh}\" $vxp=\"${port_vx}\" $ssp=\"${port_ss}\" $sop=\"${port_so}\" $anp=\"${port_an}\" $arp=\"${port_ar}\" $vlp=\"${port_vl_re}\" $vwp=\"${port_vw}\" $vmp=\"${port_vm_ws}\" $hyp=\"${port_hy2}\" $tup=\"${port_tu}\" reym=\"${ym_vl_re}\" agn=\"${ARGO_DOMAIN}\" agk=\"${ARGO_AUTH}\"; bash \"$HOME/bin/agsbx\"; fi" >> ~/.bashrc
fi
...
rm /tmp/crontab.tmp
echo "Argosbx script process started successfully, installation complete" && sleep 2
else
echo "Argosbx script process not started, installation failed" && exit
fi
}
argosbxstatus(){
echo "=========Current Status of Three Major Cores========="
procs=$(find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null)
if echo "$procs" | grep -Eq 'agsbx/s' || pgrep -f 'agsbx/s' >/dev/null 2>&1; then
echo "Sing-box (Version V$("$HOME/agsbx/sing-box" version 2>/dev/null | awk '/version/{print $NF}')): Running"
else
echo "Sing-box: Not enabled"
fi
if echo "$procs" | grep -Eq 'agsbx/x' || pgrep -f 'agsbx/x' >/dev/null 2>&1; then
echo "Xray (Version V$("$HOME/agsbx/xray" version 2>/dev/null | awk '/^Xray/{print $2}')): Running"
else
echo "Xray: Not enabled"
fi
if echo "$procs" | grep -Eq 'agsbx/c' || pgrep -f 'agsbx/c' >/dev/null 2>&1; then
echo "Argo (Version V$("$HOME/agsbx/cloudflared" version 2>/dev/null | awk '{print $3}')): Running"
else
echo "Argo: Not enabled"
fi
}
cip(){
ipbest(){
...
echo "*********************************************************
*********************************************************
Argosbx script output node configuration as follows:
"
case "$server_ip" in
104.28*|\\\[2a09*) echo "Detected WARP IP as client address (IP starting with 104.28 or 2a09). Please manually change the WARP IP in the client address to the VPS local IPv4 or IPv6 address" && sleep 3 ;; 
esac
echo
...
echo "💣【 Vless-xhttp-reality-enc 】Supports ENC encryption, node info follows:"
port_xh=$(cat "$HOME/agsbx/port_xh")
...
echo "💣【 Vless-xhttp-enc 】Supports ENC encryption, node info follows:"
port_vx=$(cat "$HOME/agsbx/port_vx")
...
echo "💣【 Vless-xhttp-ecn-cdn 】Supports ENC encryption, node info follows:"
echo "Note: Default address yg[number].ygkkk.dpdns.org. You can replace it with your preferred IP domain. If it is an origin port, manually change to 443 or 80 series port."
...
echo "💣【 Vless-ws-enc 】Supports ENC encryption, node info follows:"
port_vw=$(cat "$HOME/agsbx/port_vw")
...
echo
if [ -f "$HOME/agsbx/cdnym" ]; then
echo "💣【 Vless-ws-enc-cdn 】Supports ENC encryption, node info follows:"
echo "Note: Default address yg[number].ygkkk.dpdns.org. You can replace it with your preferred IP domain. If it is an origin port, manually change to 443 or 80 series port."
...
echo "💣【 Vless-tcp-reality-vision 】Node info follows:"
port_vl_re=$(cat "$HOME/agsbx/port_vl_re")
...
echo "💣【 Shadowsocks-2022 】Node info follows:"
port_ss=$(cat "$HOME/agsbx/port_ss")
...
echo "💣【 Vmess-ws 】Node info follows:"
port_vm_ws=$(cat "$HOME/agsbx/port_vm_ws")
...
echo
if [ -f "$HOME/agsbx/cdnym" ]; then
echo "💣【 Vmess-ws-cdn 】Node info follows:"
echo "Note: Default address yg[number].ygkkk.dpdns.org. You can replace it with your preferred IP domain. If it is an origin port, manually change to 443 or 80 series port."
...
echo "💣【 Any-Reality 】Node info follows:"
port_ar=$(cat "$HOME/agsbx/port_ar")
...
echo "💣【 Hysteria2 】Node info follows:"
port_hy2=$(cat "$HOME/agsbx/port_hy2")
...
echo "💣【 Socks5 】Client info follows:"
port_so=$(cat "$HOME/agsbx/port_so")
echo "Please use with other apps' built-in proxy, do not use as a direct node"
echo "Client Address: $server_ip"
echo "Client Port: $port_so"
echo "Client Username: $uuid"
echo "Client Password: $uuid"
echo
fi
argodomain=$(cat "$HOME/agsbx/sbargoym.log" 2>/dev/null)
...
if [ -n "$sbtk" ]; then
nametn="Argo Fixed Tunnel Token: $sbtk"
fi
argoshow=$( 
echo "Argo tunnel port is using $vlvm-ws main protocol port: $(cat $HOME/agsbx/argoport.log 2>/dev/null)
Argo Domain: $argodomain
$nametn

1、💣$vlvm-ws-tls-argo node on port 443 (Preferred IP and 443 series ports can be changed)
${vmatls_link1}${vwatls_link1}

2、💣$vlvm-ws-argo node on port 80 (Preferred IP and 80 series ports can be changed)
${vma_link7}${vwa_link2}
"
)
fi
...
echo "---------------------------------------------------------"
echo "For aggregated node info, check $HOME/agsbx/jh.txt or run cat $HOME/agsbx/jh.txt"
echo "========================================================="
echo "Related shortcuts as follows: (Need to reconnect SSH after first successful installation for agsbx shortcuts to take effect)"
showmode
}
cleandel(){
...
echo "Welcome to continue using ygkkk's Argosbx One-Click Non-Interactive Script 💣" && sleep 2
echo
showmode
exit
elif [ "$1" = "rep" ]; then
...
echo "Argosbx protocol reset complete, starting to update related protocol variables..." && sleep 2
echo
...
kill -15 $(pgrep -f 'agsbx/x' 2>/dev/null) >/dev/null 2>&1
upxray && xrestart && echo "Xray core update complete" && sleep 2 && cip
exit
elif [ "$1" = "ups" ]; then
...
kill -15 $(pgrep -f 'agsbx/s' 2>/dev/null) >/dev/null 2>&1
upsingbox && sbrestart && echo "Sing-box core update complete" && sleep 2 && cip
exit
elif [ "$1" = "res" ]; then
...
sleep 5 && echo "Restart complete" && sleep 3 && cip
exit
fi
if ! find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -Eq 'agsbx/(s|x)' && ! pgrep -f 'agsbx/(s|x)' >/dev/null 2>&1; then
...
echo "Argosbx script not installed, starting installation......" && sleep 1
if [ -n "$oap" ]; then
...
echo
echo "iptables executing open all ports"
fi
ins
cip
echo
else
echo "Argosbx script already installed"
echo
argosbxstatus
echo
echo "Related shortcuts as follows:"
showmode
exit
fi
